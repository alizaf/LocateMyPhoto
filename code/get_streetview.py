...
# getview collects images from google streetview API, using preprocessed
# lat long point that are stored in uniform_latlng_xxxx
...
class getview(object):
    ...
    def simpleclean():
        df_latlng = self.dfraw['Business_Location'].map(
    ...
    def slicepics(self, pathname,  SWslice, NEslice, newpath=None):
        onlyfiles = [f for f in listdir(pathname) if (
            isfile(join(pathname, f))) & (len(f) >= 31)]
        validfiles = [f for f in onlyfiles if (float(f[3:12]) < \
            NEslice[0]) & (float(f[3:12]) > SWslice[0])\
        & (float(f[16:27]) < NEslice[1]) & (float(f[16:27]) > SWslice[1])]
        if newpath:
            if not os.path.exists(newpath):
                os.mkdir(newpath)
            copied = [shutil.copy(pathname + f, newpath + f)
                      for f in validfiles]
    ...
    def filenameplot(filenames, plot=0):
        ...
        df = pd.DataFrame(np.transpose([latlist, lnglist, namelist]), columns=[
                          'lat', 'lng', 'filename'])
        if plot == 1:
    ...
    def creatdistinct(self, dmin, validate=False):
        ...
        if validate:
            df = pd.read_csv(self.where2store +
                             'validated.csv').dropna().reset_index()
            ...
            self.df_filt = pd.DataFrame(
                np.empty([1, len(df.columns)]), columns=[df.columns])
            self.df_alter = pd.DataFrame(
                np.empty([1, len(df.columns)]), columns=[df.columns])
            ...
            for i in range(meshsiz[1]):
                if i > 0:
                    print i * meshsiz[0] + j, 'out of', meshsiz[0] * meshsiz[1]
                for j in range(meshsiz[0]):
                    ...
                    x_sw = mesh[:, i, j][1]
                    self.df_alter.loc[
                        i * (meshsiz[0] - 1) + j] = [np.mean([y_ne, y_sw]), np.mean([x_ne, x_sw])]
                    mask = (df.lat < mesh[:, i + 1, j + 1][0]) & (df.lat > mesh[:, i, j][0]) \
                        & (df.lng < mesh[:, i + 1, j + 1][1]) & (df.lng > mesh[:, i, j][1])
                    ixlist = df[mask]
                    if ixlist.shape[0] > 0:
                        self.df_filt.loc[
                            i * (meshsiz[0] - 1) + j] = df.loc[ixlist.index[0]]
    ...
    def single_query(self, lat, lng, angle, label, ii):
        ...
        fov=%d&heading=%d&pitch=%d&source=outdoor&key=AIzaSyCFs1WFdFRhxsxqFMMKLrg2q1xcuaIFc40'\
        % (self.picsize[0],self.picsize[1], lat, lng, self.fov, angle, self.pitch)
        ...
        if os.stat(pathname).st_size in self.errsize:
            ...
            try:
                print '%d success!:%d %d Cum Success = %.3f' % (ii, self.nsucs, self.ntries, \
                    float(self.nsucs) / self.ntries)
    ...
    def relabel(self, dfl):
        ...
        for i in range(self.meshsize[0]):
            for j in range(self.meshsize[1]):
                ...
                mask = (self.df_goal.lat < mesh[:, i + 1, j + 1][0]) \
                & (self.df_goal.lat > mesh[:, i, j][0]) \
                & (self.df_goal.lng < mesh[:, i + 1, j + 1][1]) \
                & (self.df_goal.lng > mesh[:, i, j][1])
    ...
    def query(self, meshsize, picsize, angles, errsize, nattempts, fov, pitch, full=False):
        ...
        for i in range(self.df_labeled.shape[0]):
            ...
            self.angles = angles
            self.filename = self.info2name(
                self.df_labeled.lat[i], self.df_labeled.lng[i], self.angles)
            if os.path.exists(self.where2store + self.filename):
                ...
                for self.attempts in range(nattempts):
                    self.single_query(self.df_labeled.lat[i] + eps * \
                        (np.random.rand() - 1), self.df_labeled.lng[
                                      i] + eps * (np.random.rand() - 1)\
        ...
        if os.path.exists(self.where2store + 'folderdata.csv'):
            self.df_folder = self.df_labeled.append(\
                pd.read_csv(self.where2store + 'folderdata.csv')\

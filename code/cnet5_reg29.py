...
def readsplit(pathname, csv2read):
    ...
    testindex = random.sample(df.index, 100)
    testdf = df.loc[testindex[:100]]
...
class AdjustVariable(object):
    ...
    def __call__(self, nn, train_history):
        ...
        if self.ls is None:
        # if epoch <(nn.max_epochs/2):
            self.ls = np.linspace(self.start, self.stop, nn.max_epochs)
        # new_value = np.float32(self.ls[epoch - 1])
        # else:
            # self.ls = 1e-7
            # new_value = np.float32(self.ls)
        new_value = np.float32(self.ls[epoch - 1])
...
net3 = NeuralNet(
    ...
    AdjustVariable('update_momentum', start=0.001, stop=0.01),
...
pathname = './data/photodb_MainST200_20m1r/'#photodb_MainST600_40m03r/'#photodb_MainST100_25m1r#photodb_MainST_NE
...
with open('net64_1_SF_%d_25_1r_3.pickle'%picsize[0],'wb') as f:#%(SW[0],SW[1],NE[0],NE[1]), 'wb') as f:
